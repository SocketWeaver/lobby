...
public class LobbyDemo : MonoBehaviour
{
    ...
    public void RoomSettingsTest()
    {
        Debug.Log("Set ttl");
        NetworkClient.Lobby.ChangeRoomSettings(10, true, (successful,error) =>
        {
            if (successful)
            {
                Debug.Log("Changed RoomSettings!");
            }
            else
            {
                Debug.Log("Failed Changed RoomSettings " + error);
            }
        });
    }
